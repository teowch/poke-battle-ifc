...
class Pokemon
  ...
  constructor: (data) ->
    pokemon = @constructor.pokedex[data['id']]
    throw new Error("Pokemon not found: " + data['id']) unless pokemon?
    ...
    if data['moves']
      myMoves = []
      for moveId in data['moves']
        myMoves.push(new Move moveId)
      @moves = myMoves
      # console.log(@moves)
    else
  ...
  chooseMove: (defender, userMove) ->
    @move = @strategy.chooseMove defender, userMove
    # console.log @move
  ...
  modifyStatStage: (stat, change, log) ->
    log.message {changeStage: {pokemon: this.id, stage: @stats['stage']}}
